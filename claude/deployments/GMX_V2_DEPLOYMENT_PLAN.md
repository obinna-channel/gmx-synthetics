...
## Critical Step: Grant EOA CONTROLLER Role

**IMPORTANT**: After Phase 1 and before Phase 2, grant the deployer EOA the CONTROLLER role with the correct hash calculation.

Run the following script:
```bash
node scripts/marks/grant-correct-controller-role.js
```

This script uses the correct Solidity hash calculation:
```javascript
...
This temporary role is required for:
- Oracle system configuration in Phase 2
- Setting DataStore values during deployment
- Market creation and configuration

**Note**: This role should be removed from the EOA after deployment is complete for security.
