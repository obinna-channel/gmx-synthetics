...
async function main() {
    ...
    console.log("=== Creating 200k Liquidity Deposit for USDTARS Market ===\n");
    ...
    console.log("📍 Using Market 12 (USDTARS):", MARKET);
    ...
    try {
        ...
        for (const log of receipt.logs) {
            if (log.topics && log.topics.length >= 3) {
                ...
                if (potentialKey && potentialKey.length === 66 && !potentialKey.includes("000000")) {
                    ...
                    // Save to file
                    const fs = require("fs");
                    fs.writeFileSync("latest-deposit-key-usdtars-200k.txt", depositKey);
                    break;
                }
            }
        }
        ...
        if (depositKey) {
            ...
            console.log("\n📝 Saved deposit key to: latest-deposit-key-usdtars-200k.txt");
        }
