...
async function main() {
    ...
    // Get amount from environment variable (default to 100000)
    const depositAmount = process.env.AMOUNT || "100000";

    console.log("=== Creating Deposit for USDTCOP [mUSD-mUSD] Market ===\n");
    console.log("Signer address:", signer.address);
    console.log("Deposit amount:", depositAmount, "mUSD");
    ...
    const MARKET = "0x53Ab653715F2A2E3e228f17fBe120F7BEe3d7B44"; // Market 15: mUSDTCOP/mUSD/mUSD
    const mUSD = "0x85bf04B07A6df0172372b959C1C73F3e90F73faf";
    const mUSDTCOP = "0x8d9C2d46d6ff665afb4deb6CBc1Ed5E31eB455b8";
    const ROUTER = "0x6C71eD3bE6D3966F34162Cbda0195a6778096fAc";

    console.log("\n📍 Using Market 15 (USDTCOP single-token):", MARKET);
    console.log("   Index Token: mUSDTCOP (tracks USDT/COP exchange rate)");
    ...
    // Parse deposit amount (mUSD has 6 decimals)
    const musdAmount = ethers.utils.parseUnits(depositAmount, 6);
    ...
    console.log("  NOTE: Single-token market - only mUSD needed");
    console.log("  Receiver: YOUR address (normal deposit)");
    ...
    if (musdBalance.lt(musdAmount)) {
        console.log("❌ Insufficient mUSD (need " + depositAmount + " mUSD)");
        return;
    }
    ...
    // 3. Create deposit with YOUR address as receiver (normal deposit)
    // Set minMarketTokens for slippage protection (~95% of expected)
    const expectedGmTokens = musdAmount.mul(ethers.BigNumber.from(10).pow(12)); // Convert to 18 decimals
    const minMarketTokens = expectedGmTokens.mul(95).div(100); // 5% slippage tolerance

    const depositParams = {
        addresses: {
            receiver: signer.address, // YOUR address for normal deposit
        ...
        minMarketTokens: minMarketTokens, // Slippage protection
    ...
    try {
        ...
        for (const log of receipt.logs) {
            if (log.topics && log.topics.length >= 3) {
                ...
                if (potentialKey && potentialKey.length === 66 && !potentialKey.includes("000000")) {
                    ...
                    // Save to file
                    const fs = require("fs");
                    fs.writeFileSync("latest-deposit-key-usdtcop.txt", depositKey);
                    break;
                }
            }
        }
        ...
        if (depositKey) {
            console.log("\n🎯 Next steps:");
            console.log("1. Execute deposit with: npx hardhat run claude/scripts/execute-deposit-usdtcop.js --network arbitrumSepolia");
            console.log("\n📝 Saved deposit key to: latest-deposit-key-usdtcop.txt");
        }
