...
async function main() {
    ...
    console.log("=== Executing Deposit for USDTARS Market ===\n");
    ...
    let depositKey;
    try {
        depositKey = fs.readFileSync("latest-deposit-key-usdtars-market.txt", "utf8").trim();
        console.log("Deposit Key:", depositKey);
    } catch (e) {
        console.log("❌ Could not read deposit key from latest-deposit-key-usdtars-market.txt");
        console.log("   Please run create-deposit-usdtars-market.js first");
        return;
    }
    ...
    try {
        ...
        if (receipt.status) {
            ...
            if (totalSupplyAfter.gt(0)) {
                console.log("\n🎉 SUCCESS! First liquidity added to the USDTARS market!");
