...
async function main() {
    ...
    console.log("=== Creating Test SHORT Order for TSLA/USD Market ===\n");
    ...
    // Order parameters - SHORT position on TSLA
    ...
    console.log("  Type: Market Increase (SHORT TSLA) 📉");
    console.log("  Collateral: 100 mUSD");
    console.log("  Position Size: $1,000");
    console.log("  Index Token: mTSLA (at $428)");
    ...
    // 3. Create market increase order (SHORT TSLA)
    const orderParams = {
        addresses: {
        ...
        isLong: false, // SHORT TSLA position
    ...
    try {
        ...
        if (orderKey) {
            ...
            // Save to file
            const fs = require("fs");
            fs.writeFileSync("latest-order-key-tsla-short.txt", orderKey);
            console.log("📝 Saved order key to: latest-order-key-tsla-short.txt");
        }
        ...
        console.log("  3. Use MockOracleProvider for price data (mTSLA at $428)");
        console.log("  4. This will open a SHORT position - profits if TSLA price goes DOWN");
