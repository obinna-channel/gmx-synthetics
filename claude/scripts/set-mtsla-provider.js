...
async function main() {
    ...
    console.log("=== Setting mTSLA Oracle Provider ===\n");
    ...
    // Set provider for mTSLA
    console.log("\n📍 Setting provider for mTSLA...");
    const mtslaProviderKey = ethers.utils.keccak256(
        ethers.utils.defaultAbiCoder.encode(
            ["bytes32", "address", "address"],
            [ORACLE_PROVIDER_FOR_TOKEN, ORACLE, mTSLA]
        )
    );
    console.log("  Key:", mtslaProviderKey);

    try {
        // Check current provider
        const currentProvider = await dataStore.getAddress(mtslaProviderKey);
        ...
        if (currentProvider === MOCK_PROVIDER) {
            ...
            // Set the provider
            const tx = await dataStore.setAddress(mtslaProviderKey, MOCK_PROVIDER);
            await tx.wait();
            console.log("  ✅ Set to mock provider");
            console.log("  Transaction:", tx.hash);

            // Verify
            const newProvider = await dataStore.getAddress(mtslaProviderKey);
            console.log("  Verification:", newProvider === MOCK_PROVIDER ? "SUCCESS" : "FAILED");
        }
    } catch (error) {
        console.log("  ❌ Failed to set mTSLA provider:", error.message);
    }

    console.log("\n✅ mTSLA provider configuration complete!");
    console.log("\n🎯 Next step: Run set-mtsla-price.js to set the price");
}
