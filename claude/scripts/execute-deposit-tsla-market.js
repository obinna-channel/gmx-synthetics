...
async function main() {
    ...
    console.log("=== Executing Deposit for TSLA/USD Market ===\n");
    ...
    let depositKey;
    try {
        depositKey = fs.readFileSync("latest-deposit-key-tsla-market.txt", "utf8").trim();
        console.log("Deposit Key:", depositKey);
    } catch (e) {
        console.log("❌ Could not read deposit key from latest-deposit-key-tsla-market.txt");
        console.log("   Please run create-deposit-tsla-market.js first");
        return;
    }
    ...
    // For single-token market: index token (mTSLA) + collateral token (mUSD)
    const oracleParams = {
        tokens: [mTSLA, mUSD],
    ...
    try {
        ...
        if (receipt.status) {
            ...
            if (totalSupplyAfter.gt(0)) {
                console.log("\n🎉 SUCCESS! First liquidity added to the TSLA/USD market!");
