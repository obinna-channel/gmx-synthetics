...
async function main() {
    ...
    console.log("=== Executing Deposit for NVDA/USD Market ===\n");
    ...
    try {
        ...
        console.log("   Please run create-deposit-nvda-market.js first");
        return;
    }
    ...
    // For single-token market: index token (mNVDA) + collateral token (mUSD)
    const oracleParams = {
        tokens: [mNVDA, mUSD],
    ...
    try {
        ...
        if (receipt.status) {
            ...
            if (totalSupplyAfter.gt(0)) {
                console.log("\n🎉 SUCCESS! First liquidity added to the NVDA/USD market!");
