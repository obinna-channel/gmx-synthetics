...
async function main() {
    ...
    console.log("=== Executing Deposit for PKR [mUSD-mUSD] Market ===\n");
    ...
    let depositKey;
    try {
        depositKey = fs.readFileSync("latest-deposit-key-pkr.txt", "utf8").trim();
        console.log("Deposit Key:", depositKey);
    } catch (e) {
        console.log("❌ Could not read deposit key from latest-deposit-key-pkr.txt");
        console.log("   Please run create-deposit-pkr-amount.js first");
        return;
    }
    ...
    try {
        ...
        if (receipt.status) {
            ...
            if (totalSupplyAfter.gt(0)) {
                console.log("\n🎉 SUCCESS! Liquidity added to the PKR [mUSD-mUSD] market!");
            } else {
