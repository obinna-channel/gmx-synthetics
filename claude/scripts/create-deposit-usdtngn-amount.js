...
async function main() {
    ...
    // Get amount from environment variable (default to 33000)
    const depositAmount = process.env.AMOUNT || "33000";

    console.log("=== Creating Deposit for USDTNGN [mUSD-mUSD] Market ===\n");
    ...
    const MARKET = "0x1aF0891884AD96De1Cb1CC3fDEd67842F00926bb"; // Market 18: mUSDTNGN/mUSD/mUSD
    ...
    console.log("\n📍 Using Market 18 (USDTNGN single-token):", MARKET);
    ...
    try {
        ...
        for (const log of receipt.logs) {
            if (log.topics && log.topics.length >= 3) {
                ...
                if (potentialKey && potentialKey.length === 66 && !potentialKey.includes("000000")) {
                    ...
                    // Save to file
                    const fs = require("fs");
                    fs.writeFileSync("latest-deposit-key-usdtngn.txt", depositKey);
                    break;
                }
            }
        }
        ...
        if (depositKey) {
            console.log("\n🎯 Next steps:");
            console.log("1. Execute deposit with: npx hardhat run claude/scripts/execute-deposit-usdtngn-single-market.js --network arbitrumSepolia");
            console.log("\n📝 Saved deposit key to: latest-deposit-key-usdtngn.txt");
        }
