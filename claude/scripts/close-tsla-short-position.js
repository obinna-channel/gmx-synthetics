...
async function main() {
    ...
    console.log("=== Closing TSLA SHORT Position ===\n");
    ...
    console.log("📍 Market 11 (TSLA/USD):", MARKET);
    console.log("   Index Token: mTSLA");
    ...
    if (!positionExists) {
        console.log("❌ No short position exists to close");
        return;
    }
    ...
    // 2. Create market decrease order to close SHORT position
    const orderParams = {
        addresses: {
        ...
        numbers: {
            ...
            triggerPrice: 0,
            acceptablePrice: ethers.utils.parseUnits("1000", 12), // Max acceptable price for closing short ($1000 per TSLA)
    ...
    // Execute transaction
    console.log("📍 Creating close order for SHORT position...");
    ...
    try {
        ...
        if (orderKey) {
            ...
            // Save to file
            const fs = require("fs");
            fs.writeFileSync("latest-close-order-key-tsla-short.txt", orderKey);
            console.log("📝 Saved order key to: latest-close-order-key-tsla-short.txt");
        }
        ...
        console.log("  3. SHORT position will be closed and collateral returned");
