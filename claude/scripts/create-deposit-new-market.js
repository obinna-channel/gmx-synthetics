...
async function main() {
    ...
    try {
        ...
        for (const log of receipt.logs) {
            if (log.topics && log.topics.length >= 3) {
                ...
                if (potentialKey && potentialKey.length === 66 && !potentialKey.includes("000000")) {
                    ...
                    // Save to file
                    const fs = require("fs");
                    fs.writeFileSync("latest-deposit-key-new-market.txt", depositKey);
                    break;
                }
            }
        }
        ...
        if (depositKey) {
            console.log("\n🎯 Next steps:");
            console.log("1. Run set-fresh-prices.js to set oracle prices");
            console.log("2. Execute deposit with this key");
            console.log("\n📝 Saved deposit key to: latest-deposit-key-new-market.txt");
        }
