...
async function main() {
    ...
    console.log("=== Creating Deposit for PKR [mUSD-mUSD] Market ===\n");
    ...
    console.log("\n📍 Using Market 14 (PKR single-token):", MARKET);
    console.log("   Index Token: mPKR (tracks USDT/PKR exchange rate)");
    ...
    try {
        ...
        for (const log of receipt.logs) {
            if (log.topics && log.topics.length >= 3) {
                ...
                if (potentialKey && potentialKey.length === 66 && !potentialKey.includes("000000")) {
                    ...
                    // Save to file
                    const fs = require("fs");
                    fs.writeFileSync("latest-deposit-key-pkr.txt", depositKey);
                    break;
                }
            }
        }
        ...
        if (depositKey) {
            console.log("\n🎯 Next steps:");
            console.log("1. Execute deposit with: npx hardhat run claude/scripts/execute-deposit-pkr.js --network arbitrumSepolia");
            console.log("\n📝 Saved deposit key to: latest-deposit-key-pkr.txt");
        }
