...
async function main() {
    ...
    console.log("=== Creating 200k mUSD Deposit for NVDA/USD Market ===\n");
    ...
    console.log("📍 Using Market 13 (NVDA/USD):", MARKET);
    console.log("   Index Token: mNVDA (tracks NVDA stock price)");
    ...
    try {
        ...
        for (const log of receipt.logs) {
            if (log.topics && log.topics.length >= 3) {
                ...
                if (potentialKey && potentialKey.length === 66 && !potentialKey.includes("000000")) {
                    ...
                    const fs = require("fs");
                    fs.writeFileSync("latest-deposit-key-nvda-200k.txt", depositKey);
                    break;
                }
            }
        }
        ...
        if (depositKey) {
            console.log("\n🎯 Next steps:");
            console.log("1. Execute deposit with: npx hardhat run claude/scripts/execute-deposit-nvda-market.js --network arbitrumSepolia");
            console.log("   (Update the script to read from latest-deposit-key-nvda-200k.txt)");
            console.log("\n📝 Saved deposit key to: latest-deposit-key-nvda-200k.txt");
        }
