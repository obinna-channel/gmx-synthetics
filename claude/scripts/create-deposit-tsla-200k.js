...
async function main() {
    ...
    console.log("=== Creating 200k mUSD Deposit for TSLA/USD Market ===\n");
    ...
    const MARKET = "0x8ae559448a1482faffC925eF6a233276588348Df"; // Market 11: mTSLA/mUSD/mUSD
    ...
    console.log("📍 Using Market 11 (TSLA/USD):", MARKET);
    console.log("   Index Token: mTSLA (tracks TSLA stock price)");
    ...
    // Deposit amounts - 200,000 mUSD
    const musdAmount = ethers.utils.parseUnits("200000", 6); // 200k mUSD
    ...
    console.log("  Receiver: YOUR ADDRESS (normal deposit, not address(1))");
    ...
    console.log("  2. sendTokens - Send mUSD (200,000)");
    ...
    const depositParams = {
        addresses: {
            receiver: signer.address, // YOUR address (not address(1))
            callbackContract: ethers.constants.AddressZero,
            uiFeeReceiver: ethers.constants.AddressZero,
            market: MARKET,
            initialLongToken: mUSD,
            initialShortToken: mUSD, // Same as long (single-token market)
            longTokenSwapPath: [],
            shortTokenSwapPath: []
        },
        minMarketTokens: 0, // Keep at 0 for safety
    ...
    try {
        ...
        for (const log of receipt.logs) {
            if (log.topics && log.topics.length >= 3) {
                ...
                if (potentialKey && potentialKey.length === 66 && !potentialKey.includes("000000")) {
                    ...
                    // Save to file (overwrite the previous one)
                    const fs = require("fs");
                    fs.writeFileSync("latest-deposit-key-tsla-market.txt", depositKey);
                    break;
                }
            }
        }
        ...
        if (depositKey) {
            console.log("\n🎯 Next steps:");
            console.log("1. Execute deposit with: npx hardhat run claude/scripts/execute-deposit-tsla-market.js --network arbitrumSepolia");
            console.log("\n📝 Saved deposit key to: latest-deposit-key-tsla-market.txt");
        }
