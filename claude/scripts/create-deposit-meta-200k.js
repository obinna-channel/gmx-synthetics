...
async function main() {
    ...
    console.log("=== Creating 200k mUSD Deposit for META/USD Market ===\n");
    ...
    const MARKET = "0xafd908D358315efDBA493311AbE30648DEC4d2dE"; // Market 17: mMETA/mUSD/mUSD
    ...
    console.log("📍 Using Market 17 (META/USD):", MARKET);
    console.log("   Index Token: mMETA (tracks META stock price)");
    ...
    try {
        ...
        for (const log of receipt.logs) {
            if (log.topics && log.topics.length >= 3) {
                ...
                if (potentialKey && potentialKey.length === 66 && !potentialKey.includes("000000")) {
                    ...
                    const fs = require("fs");
                    fs.writeFileSync("latest-deposit-key-meta-market.txt", depositKey);
                    break;
                }
            }
        }
        ...
        if (depositKey) {
            console.log("\n🎯 Next steps:");
            console.log("1. Execute deposit with: npx hardhat run claude/scripts/execute-deposit-meta-market.js --network arbitrumSepolia");
            console.log("\n📝 Saved deposit key to: latest-deposit-key-meta-market.txt");
        }
