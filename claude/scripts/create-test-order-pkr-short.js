...
async function main() {
    ...
    console.log("=== Creating Test SHORT Order for PKR Market ===\n");
    ...
    // Order parameters - SHORT position on PKR
    ...
    console.log("  Type: Market Increase (SHORT PKR) 📉");
    ...
    // 3. Create market increase order (SHORT PKR)
    const orderParams = {
        addresses: {
        ...
        isLong: false, // SHORT PKR position
    ...
    try {
        ...
        if (orderKey) {
            ...
            // Save to file
            const fs = require("fs");
            fs.writeFileSync("latest-order-key-pkr-short.txt", orderKey);
            console.log("📝 Saved order key to: latest-order-key-pkr-short.txt");
        }
        ...
        console.log("  4. This will open a SHORT position - profits if PKR price goes DOWN");
