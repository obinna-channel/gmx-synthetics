...
async function fetchPriceFromAPI() {
    const API_URL = "https://marks-server-a58cc19eb539.herokuapp.com/api/v1/price/current/USDTARS";
...
async function main() {
    ...
    console.log("=== Setting mUSDTARS and mUSD Prices in MockOracleProvider ===\n");
    console.log("Signer:", signer.address);

    const MOCK_PROVIDER = "0x5D85d4acd35ffD0daD76C5eB0da3d7e53e20cCC5";
    const mUSDTARS = "0xed6890bE2409F0db06a00C809a298E2E06553BE1"; // Index token (18 decimals)
    const mUSD = "0x85bf04B07A6df0172372b959C1C73F3e90F73faf"; // Collateral token (6 decimals)

    // Fetch current exchange rate from API
    console.log("📡 Fetching current USDTARS price from API...");
    let EXCHANGE_RATE;
    try {
        EXCHANGE_RATE = await fetchPriceFromAPI();
        console.log(`✅ Fetched rate: 1 USD = ${EXCHANGE_RATE} ARS\n`);
    } catch (error) {
        console.log(`❌ Failed to fetch price from API: ${error.message}`);
        console.log("   Using fallback rate: 1 USD = 1000 ARS\n");
        EXCHANGE_RATE = 1000;
    }
    ...
    console.log("\nPrice Configuration:");
    console.log("  mUSDTARS (index): " + EXCHANGE_RATE + " (tracks USDT/ARS exchange rate)");
    ...
    // mUSDTARS: 18 decimals
    // Price = exchange_rate * 10^(30-18) = exchange_rate * 10^12
    // Handle decimal by converting to integer representation
    // e.g., 1000.50 -> 100050 * 10^10
    const rateInteger = Math.floor(EXCHANGE_RATE * 100); // Convert to integer (e.g., 1000.50 -> 100050)
    const musdtArsPrice = ethers.BigNumber.from(rateInteger).mul(ethers.BigNumber.from(10).pow(10));
    ...
    try {
        // Set mUSDTARS price
        console.log("\n📍 Step 1: Setting mUSDTARS price...");
        const tx1 = await mockProvider.setPriceWithPrecision(mUSDTARS, musdtArsPrice);
        console.log("  TX sent:", tx1.hash);
        await tx1.wait();
        console.log("  ✅ mUSDTARS price set to", EXCHANGE_RATE);
        ...
        console.log("\n🎯 Next steps:");
        console.log("  1. Create deposit with: AMOUNT=100000 npx hardhat run claude/scripts/create-deposit-usdtars-amount.js --network arbitrumSepolia");
        console.log("  2. Execute deposit with: npx hardhat run claude/scripts/execute-deposit-usdtars.js --network arbitrumSepolia");
