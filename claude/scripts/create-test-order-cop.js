...
async function main() {
    ...
    console.log("=== Creating Test Order for COP Market ===\n");
    ...
    // Order parameters - Long position on COP
    ...
    console.log("  Type: Market Increase (Long COP)");
    ...
    // 3. Create market increase order (Long COP)
    // Acceptable price: max 4500 COP per USDT (allowing for price movement)
    const acceptablePrice = ethers.utils.parseUnits("4500", 12); // 4500 * 10^12 for 18 decimal token
    ...
    const orderParams = {
        ...
        numbers: {
            ...
            acceptablePrice: acceptablePrice, // Max acceptable price for COP
    ...
    try {
        ...
        if (orderKey) {
            ...
            // Save to file
            const fs = require("fs");
            fs.writeFileSync("latest-order-key-cop.txt", orderKey);
            console.log("📝 Saved order key to: latest-order-key-cop.txt");
        }
