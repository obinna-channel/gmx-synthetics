...
async function main() {
    ...
    console.log("=== Executing Deposit for USDTNGN [mUSD-mUSD] Market ===\n");
    ...
    let depositKey;
    try {
        depositKey = fs.readFileSync("latest-deposit-key-usdtngn.txt", "utf8").trim();
        console.log("Deposit Key:", depositKey);
    } catch (e) {
        console.log("❌ Could not read deposit key from latest-deposit-key-usdtngn.txt");
        console.log("   Please run create-deposit-usdtngn-amount.js first");
        return;
    }
    ...
    if (!isInList) {
        ...
        console.log("   Please run create-deposit-usdtngn-single-market.js first");
        return;
    }
    ...
    // For single-token market: index token (mUSDTNGN) + collateral token (mUSD)
    const oracleParams = {
        tokens: [mUSDTNGN, mUSD],
    ...
    try {
        ...
        if (receipt.status) {
            ...
            if (totalSupplyAfter.gt(0)) {
                console.log("\n🎉 SUCCESS! First liquidity added to the USDTNGN [mUSD-mUSD] market!");
