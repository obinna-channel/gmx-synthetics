...
async function main() {
    ...
    console.log("=== Executing Deposit for COP Market ===\n");
    ...
    let depositKey;
    try {
        depositKey = fs.readFileSync("latest-deposit-key-cop-market.txt", "utf8").trim();
        console.log("Deposit Key:", depositKey);
    } catch (e) {
        console.log("❌ Could not read deposit key from latest-deposit-key-cop-market.txt");
        console.log("   Please run create-deposit-cop-market.js first");
        return;
    }
    ...
    // For single-token market: index token (mCOP) + collateral token (mUSD)
    const oracleParams = {
        tokens: [mCOP, mUSD],
    ...
    try {
        ...
        if (receipt.status) {
            ...
            if (totalSupplyAfter.gt(0)) {
                console.log("\n🎉 SUCCESS! First liquidity added to the COP market!");
                console.log("The market is now initialized!");
            } else {
