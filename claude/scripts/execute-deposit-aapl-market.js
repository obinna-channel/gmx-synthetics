...
async function main() {
    ...
    console.log("=== Executing Deposit for AAPL/USD Market ===\n");
    ...
    let depositKey;
    try {
        depositKey = fs.readFileSync("latest-deposit-key-aapl-market.txt", "utf8").trim();
        console.log("Deposit Key:", depositKey);
    } catch (e) {
        console.log("❌ Could not read deposit key from latest-deposit-key-aapl-market.txt");
        console.log("   Please run create-deposit-aapl-market.js first");
        return;
    }
    ...
    // For single-token market: index token (mAAPL) + collateral token (mUSD)
    const oracleParams = {
        tokens: [mAAPL, mUSD],
    ...
    try {
        ...
        if (receipt.status) {
            ...
            if (totalSupplyAfter.gt(0)) {
                console.log("\n🎉 SUCCESS! First liquidity added to the AAPL/USD market!");
