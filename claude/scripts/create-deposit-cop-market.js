...
async function main() {
    ...
    console.log("=== Creating First Deposit for COP Market ===\n");
    ...
    try {
        ...
        for (const log of receipt.logs) {
            if (log.topics && log.topics.length >= 3) {
                ...
                if (potentialKey && potentialKey.length === 66 && !potentialKey.includes("000000")) {
                    ...
                    // Save to file
                    const fs = require("fs");
                    fs.writeFileSync("latest-deposit-key-cop-market.txt", depositKey);
                    break;
                }
            }
        }
        ...
        if (depositKey) {
            console.log("\n🎯 Next steps:");
            console.log("1. Execute deposit with: npx hardhat run claude/scripts/execute-deposit-cop-market.js --network arbitrumSepolia");
            console.log("\n📝 Saved deposit key to: latest-deposit-key-cop-market.txt");
        }
