...
async function main() {
    ...
    console.log("=== Executing Deposit for META/USD Market ===\n");
    ...
    let depositKey;
    try {
        depositKey = fs.readFileSync("latest-deposit-key-meta-market.txt", "utf8").trim();
        console.log("Deposit Key:", depositKey);
    } catch (e) {
        console.log("❌ Could not read deposit key from latest-deposit-key-meta-market.txt");
        console.log("   Please run create-deposit-meta-market.js first");
        return;
    }
    ...
    // For single-token market: index token (mMETA) + collateral token (mUSD)
    const oracleParams = {
        tokens: [mMETA, mUSD],
    ...
    try {
        ...
        if (receipt.status) {
            ...
            if (totalSupplyAfter.gt(0)) {
                console.log("\n🎉 SUCCESS! First liquidity added to the META/USD market!");
