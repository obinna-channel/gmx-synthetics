...
async function main() {
    ...
    console.log("=== Executing Deposit for USDTCOP [mUSD-mUSD] Market ===\n");
    ...
    let depositKey;
    try {
        depositKey = fs.readFileSync("latest-deposit-key-usdtcop.txt", "utf8").trim();
        console.log("Deposit Key:", depositKey);
    } catch (e) {
        console.log("❌ Could not read deposit key from latest-deposit-key-usdtcop.txt");
        console.log("   Please run create-deposit-usdtcop-amount.js first");
        return;
    }
    ...
    // For single-token market: index token (mUSDTCOP) + collateral token (mUSD)
    const oracleParams = {
        tokens: [mUSDTCOP, mUSD],
    ...
    try {
        ...
        if (receipt.status) {
            ...
            if (totalSupplyAfter.gt(0)) {
                console.log("\n🎉 SUCCESS! Liquidity added to the USDTCOP [mUSD-mUSD] market!");
            } else {
