...
async function main() {
    ...
    console.log("=== Creating Test Order for USDTARS Market ===\n");
    ...
    console.log("📍 Market 12 (USDTARS):", MARKET);
    console.log("   Index Token: mUSDTARS");
    ...
    // Order parameters - Long position on USDTARS
    // Collateral: 100 mUSD (6 decimals)
    // Position size: $1,000 USD (100 mUSD * 10x leverage)
    // This creates a 10x leverage position
    ...
    console.log("  Type: Market Increase (Long USDTARS)");
    console.log("  Collateral: 100 mUSD");
    console.log("  Position Size: $1,000");
    console.log("  Index Token: mUSDTARS (at 1458.51 ARS per USDT)");
    ...
    // 3. Create market increase order (Long USDTARS)
    // Acceptable price: max 2000 ARS per USDT (allowing for price movement)
    const acceptablePrice = ethers.utils.parseUnits("2000", 12); // 2000 * 10^12 for 18 decimal token
    ...
    const orderParams = {
        ...
        numbers: {
            ...
            acceptablePrice: acceptablePrice, // Max acceptable price for USDTARS
        ...
        isLong: true, // Long USDTARS position
    ...
    try {
        ...
        if (orderKey) {
            ...
            // Save to file
            const fs = require("fs");
            fs.writeFileSync("latest-order-key-usdtars.txt", orderKey);
            console.log("📝 Saved order key to: latest-order-key-usdtars.txt");
        }
        ...
        console.log("  3. Use MockOracleProvider for price data (mUSDTARS at 1458.51)");
