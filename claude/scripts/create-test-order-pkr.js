...
async function main() {
    ...
    console.log("=== Creating Test Order for PKR Market ===\n");
    ...
    // Order parameters - Long position on PKR
    ...
    console.log("  Type: Market Increase (Long PKR)");
    console.log("  Collateral: 100 mUSD");
    console.log("  Position Size: $1,000");
    console.log("  Index Token: mPKR (at 289.71 PKR per USDT)");
    ...
    // 3. Create market increase order (Long PKR)
    // Acceptable price: max 350 PKR per USDT (allowing for price movement)
    const acceptablePrice = ethers.utils.parseUnits("350", 12); // 350 * 10^12 for 18 decimal token
    ...
    const orderParams = {
        ...
        numbers: {
            ...
            acceptablePrice: acceptablePrice, // Max acceptable price for PKR
    ...
    try {
        ...
        if (orderKey) {
            ...
            // Save to file
            const fs = require("fs");
            fs.writeFileSync("latest-order-key-pkr.txt", orderKey);
            console.log("📝 Saved order key to: latest-order-key-pkr.txt");
        }
        ...
        console.log("  3. Use MockOracleProvider for price data (mPKR at 289.71)");
