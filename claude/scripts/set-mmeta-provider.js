...
async function main() {
    ...
    console.log("=== Setting mMETA Oracle Provider ===\n");
    ...
    // Set provider for mMETA
    console.log("\n📍 Setting provider for mMETA...");
    const mmetaProviderKey = ethers.utils.keccak256(
        ethers.utils.defaultAbiCoder.encode(
            ["bytes32", "address", "address"],
            [ORACLE_PROVIDER_FOR_TOKEN, ORACLE, mMETA]
        )
    );
    console.log("  Key:", mmetaProviderKey);

    try {
        // Check current provider
        const currentProvider = await dataStore.getAddress(mmetaProviderKey);
        ...
        if (currentProvider === MOCK_PROVIDER) {
            ...
            // Set the provider
            const tx = await dataStore.setAddress(mmetaProviderKey, MOCK_PROVIDER);
            await tx.wait();
            console.log("  ✅ Set to mock provider");
            console.log("  Transaction:", tx.hash);

            // Verify
            const newProvider = await dataStore.getAddress(mmetaProviderKey);
            console.log("  Verification:", newProvider === MOCK_PROVIDER ? "SUCCESS" : "FAILED");
        }
    } catch (error) {
        console.log("  ❌ Failed to set mMETA provider:", error.message);
    }

    console.log("\n✅ mMETA provider configuration complete!");
    console.log("\n🎯 Next step: Run set-mmeta-price.js to set the price");
}
