...
async function main() {
    ...
    console.log("=== Creating 200k Liquidity Deposit for COP Market ===\n");
    ...
    console.log("📍 Using Market 15 (COP):", MARKET);
    console.log("   Index Token: mCOP (tracks USDT/COP exchange rate)");
    ...
    try {
        ...
        for (const log of receipt.logs) {
            if (log.topics && log.topics.length >= 3) {
                ...
                if (potentialKey && potentialKey.length === 66 && !potentialKey.includes("000000")) {
                    ...
                    // Save to file
                    const fs = require("fs");
                    fs.writeFileSync("latest-deposit-key-cop-200k.txt", depositKey);
                    break;
                }
            }
        }
        ...
        if (depositKey) {
            ...
            console.log("\n📝 Saved deposit key to: latest-deposit-key-cop-200k.txt");
        }
