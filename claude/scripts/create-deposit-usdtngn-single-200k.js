...
async function main() {
    ...
    console.log("=== Creating 200k Liquidity Deposit for USDTNGN [mUSD-mUSD] Market ===\n");
    ...
    console.log("📍 Using Market 18 (USDTNGN single-token):", MARKET);
    ...
    // Deposit amounts - 200k mUSD for liquidity
    const musdAmount = ethers.utils.parseUnits("200000", 6); // 200,000 mUSD
    ...
    console.log("  NOTE: This is the liquidity deposit (NOT the first deposit)");
    console.log("  Receiver: YOUR address (not address(1))");
    ...
    if (musdBalance.lt(musdAmount)) {
        console.log("❌ Insufficient mUSD (need 200,000 mUSD)");
        return;
    }
    ...
    // 3. Create deposit with YOUR address as receiver (not address(1))
    // Set minMarketTokens for slippage protection (~95% of expected)
    const minMarketTokens = ethers.utils.parseUnits("190000", 18); // Expect ~200k, min 190k

    const depositParams = {
        addresses: {
            receiver: signer.address, // YOUR address for this deposit
            callbackContract: ethers.constants.AddressZero,
            uiFeeReceiver: ethers.constants.AddressZero,
            market: MARKET, // USDTNGN [mUSD-mUSD] market
    ...
    try {
        ...
        for (const log of receipt.logs) {
            if (log.topics && log.topics.length >= 3) {
                ...
                if (potentialKey && potentialKey.length === 66 && !potentialKey.includes("000000")) {
                    ...
                    // Save to file
                    const fs = require("fs");
                    fs.writeFileSync("latest-deposit-key-usdtngn-single-200k.txt", depositKey);
                    break;
                }
            }
        }
        ...
        if (depositKey) {
            console.log("\n🎯 Next steps:");
            console.log("1. Execute deposit with keeper or manually");
            console.log("2. The keeper should automatically execute this deposit");
            console.log("\n📝 Saved deposit key to: latest-deposit-key-usdtngn-single-200k.txt");
        }
