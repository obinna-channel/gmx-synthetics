...
async function readPositionsViaReader() {
    ...
    console.log(`Market: ${colors.yellow}mUSD/mUSD/mNGN (Market #7)${colors.reset}`);
        ...
        for (const isLong of [true, false]) {
            try {
                ...
                if (position && position.numbers && position.numbers.sizeInUsd && position.numbers.sizeInUsd.gt(0)) {
                    ...
                    console.log(`    Size in Tokens (mUSD): ${colors.yellow}${ethers.utils.formatUnits(position.numbers.sizeInTokens, 6)}${colors.reset}`);
    ...
    try {
        ...
        if (market) {
            ...
            console.log(`Index Token: ${colors.yellow}${market.indexToken}${colors.reset} (mUSD)`);
            console.log(`Long Token: ${colors.yellow}${market.longToken}${colors.reset} (mUSD)`);
            console.log(`Short Token: ${colors.yellow}${market.shortToken}${colors.reset} (mNGN)`);
