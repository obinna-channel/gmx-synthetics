...
async function main() {
    ...
    console.log("=== Creating Normal Deposit for mUSDTNGN/mUSD/mNGN Market ===\n");
    console.log("Adding liquidity to existing market\n");
    ...
    // Verify market has liquidity (required for normal deposit)
    ...
    if (totalSupply.eq(0)) {
        console.log("❌ Market has no liquidity! Need to do first deposit with address(1)");
        return;
    }
    console.log("✅ Market has liquidity, can proceed with normal deposit\n");
    ...
    // Large deposit amounts for significant liquidity
    const musdAmount = ethers.utils.parseUnits("100000", 6); // 100,000 mUSD
    const mngnAmount = ethers.utils.parseUnits("150000000", 18); // 150,000,000 mNGN (equals 100k mUSD at 1:1500 rate)
    ...
    console.log("  Total liquidity: $200,000 USD");
    console.log("  Execution fee:", ethers.utils.formatEther(executionFee), "ETH");
    console.log("  Receiver:", signer.address, "(YOUR ADDRESS - for normal deposit)");
    ...
    console.log("  4. createDeposit with YOUR address as receiver (normal deposit)");
    ...
    const depositParams = {
        ...
        minMarketTokens: 0, // Can set minimum expected for slippage protection
    ...
    try {
        ...
        for (const log of receipt.logs) {
            if (log.topics && log.topics.length >= 3) {
                ...
                if (potentialKey && potentialKey.length === 66 && !potentialKey.includes("000000")) {
                    ...
                    // Save to file
                    const fs = require("fs");
                    fs.writeFileSync("latest-musdtngn-normal-deposit-key.txt", depositKey);
                    break;
                }
            }
        }
        ...
        if (depositKey) {
            console.log("\n🎯 Next steps:");
            console.log("1. Execute deposit with: npx hardhat run scripts/execute-musdtngn-normal-deposit.js --network arbitrumSepolia");
            console.log("\n📝 Saved deposit key to: latest-musdtngn-normal-deposit-key.txt");
        }
