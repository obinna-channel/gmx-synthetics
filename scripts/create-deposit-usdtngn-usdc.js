...
async function main() {
    ...
    console.log("=== Creating Normal Deposit for mUSDTNGN [USDC-USDC] Market ===\n");
    ...
    // Check if market has liquidity (first deposit must be done first)
    const totalSupply = await marketToken.totalSupply();
    if (totalSupply.eq(0)) {
        console.log("❌ Market has no liquidity!");
        console.log("   You must run create-first-deposit-usdtngn-usdc.js first");
        console.log("   Then execute it with execute-deposit-usdtngn-usdc.js");
        return;
    }
    console.log("✅ Market has liquidity:", ethers.utils.formatEther(totalSupply), "GM tokens");
    ...
    console.log("  Receiver: YOUR address (you will receive LP tokens)");
    ...
    console.log("  3. createDeposit (NORMAL DEPOSIT - receiver = your address)");
    ...
    // 3. Create NORMAL deposit with user's address as receiver
    // Set minMarketTokens for slippage protection (~95% of expected)
    const expectedGmTokens = usdcAmount.mul(ethers.BigNumber.from(10).pow(12)); // Convert to 18 decimals
