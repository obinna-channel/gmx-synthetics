...
async function main() {
    ...
    console.log("=== Creating Normal Deposit for mUSD/mUSD/mNGN Market ===\n");
    ...
    console.log("  Total liquidity: 300,000,000 NGN");
    ...
    try {
        ...
        for (const log of receipt.logs) {
            if (log.topics && log.topics.length >= 3) {
                ...
                if (potentialKey && potentialKey.length === 66 && !potentialKey.includes("000000")) {
                    ...
                    // Save to file
                    const fs = require("fs");
                    fs.writeFileSync("latest-musd-normal-deposit-key.txt", depositKey);
                    break;
                }
            }
        }
        ...
        if (depositKey) {
            console.log("\n🎯 Next steps:");
            console.log("1. Execute deposit with: npx hardhat run scripts/execute-musd-normal-deposit.js --network arbitrumSepolia");
            console.log("\n📝 Saved deposit key to: latest-musd-normal-deposit-key.txt");
        }
