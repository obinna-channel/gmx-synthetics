...
// Contract addresses for mUSD/mNGN Market
const ADDRESSES = {
...
${colors.bright}USAGE:${colors.reset}
  ACTION=<action> [OPTIONS] npx hardhat run scripts/test-musd-market-orders.js --network arbitrumSepolia
...
${colors.bright}OPTIONS (as environment variables):${colors.reset}
  ...
  SIZE=<number>          Size in NGN (required for open/increase)
  ...
  AMOUNT=<number>        Specific NGN amount to decrease
  TOKEN=<mUSD|mNGN>      Token for price update
  PRICE=<number>         New price in NGN

${colors.bright}MARKET INFO:${colors.reset}
  Market: ${colors.yellow}mUSD/mUSD/mNGN (Market #7)${colors.reset}
  Index Token: mUSD
  Long Token: mUSD (collateral for longs)
  Short Token: mNGN (collateral for shorts)
  Pricing: Exchange rate (1 mUSD = 1500 NGN, 1 mNGN = 1 NGN)

${colors.bright}EXAMPLES:${colors.reset}
  # Check current positions
  ACTION=check npx hardhat run scripts/test-musd-market-orders.js --network arbitrumSepolia

  # Open a long position (100,000 NGN size with 100 mUSD collateral)
  ACTION=open SIDE=long SIZE=100000 COLLATERAL=100 npx hardhat run scripts/test-musd-market-orders.js --network arbitrumSepolia

  # Open a short position (75,000 NGN size with 75,000 mNGN collateral)
  ACTION=open SIDE=short SIZE=75000 COLLATERAL=75000 npx hardhat run scripts/test-musd-market-orders.js --network arbitrumSepolia

  # Increase a position
  ACTION=increase SIDE=long SIZE=50000 COLLATERAL=50 npx hardhat run scripts/test-musd-market-orders.js --network arbitrumSepolia

  # Decrease 50% of position
  ACTION=decrease SIDE=long PERCENT=50 npx hardhat run scripts/test-musd-market-orders.js --network arbitrumSepolia

  # Decrease specific amount
  ACTION=decrease SIDE=short AMOUNT=25000 npx hardhat run scripts/test-musd-market-orders.js --network arbitrumSepolia

  # Close entire position
  ACTION=close SIDE=long npx hardhat run scripts/test-musd-market-orders.js --network arbitrumSepolia

  # Update oracle price
  ACTION=set-price TOKEN=mUSD PRICE=1600 npx hardhat run scripts/test-musd-market-orders.js --network arbitrumSepolia
`);
}
...
async function checkPosition(signer, side = null) {
    ...
    console.log(`Market: ${colors.yellow}mUSD/mUSD/mNGN${colors.reset}`);
    ...
    for (const checkSide of sidesToCheck) {
        ...
        if (positionExists) {
            ...
            console.log(`  Size: ${colors.green}${ethers.utils.formatUnits(sizeInUsd, 30)} NGN${colors.reset}`);

            if (isLong) {
                console.log(`  Collateral: ${colors.yellow}${ethers.utils.formatUnits(collateralAmount, 6)} mUSD${colors.reset}`);
                // Calculate leverage (size in NGN / collateral in mUSD * 1500)
                if (collateralAmount.gt(0)) {
                    const collateralInNgn = collateralAmount.mul(1500).mul(ethers.utils.parseUnits("1", 24));
                    const leverage = sizeInUsd.div(collateralInNgn);
                    console.log(`  Leverage: ${colors.magenta}~${leverage.toString()}x${colors.reset}`);
                }
            } else {
                console.log(`  Collateral: ${colors.yellow}${ethers.utils.formatUnits(collateralAmount, 18)} mNGN${colors.reset}`);
                // Calculate leverage (size in NGN / collateral in mNGN)
                if (collateralAmount.gt(0)) {
                    const leverage = sizeInUsd.div(collateralAmount.mul(ethers.utils.parseUnits("1", 12)));
                    console.log(`  Leverage: ${colors.magenta}~${leverage.toString()}x${colors.reset}`);
                }
            }
        } else {
...
async function createOrder(signer, orderType, orderParams) {
    ...
    const collateralToken = orderParams.isLong ?
        await ethers.getContractAt("IERC20", ADDRESSES.mUSD) :
        await ethers.getContractAt("IERC20", ADDRESSES.mNGN);
    ...
    if (orderType === OrderTypes.MarketIncrease && orderParams.collateralAmount) {
        const collateralAddress = orderParams.isLong ? ADDRESSES.mUSD : ADDRESSES.mNGN;
        ...
        if (allowance.lt(orderParams.collateralAmount)) {
            console.log(`  ${colors.yellow}Approving ${orderParams.isLong ? 'mUSD' : 'mNGN'}...${colors.reset}`);
    ...
    // 3. Create order
    // For decrease orders with positions, we need to handle swaps appropriately
    ...
    const createOrderParams = {
        addresses: {
            ...
            initialCollateralToken: orderParams.isLong ? ADDRESSES.mUSD : ADDRESSES.mNGN,
            swapPath: []
        },
        numbers: {
...
async function openPosition(signer, options) {
    ...
    if (!side || !size || !collateral) {
        ...
        console.log("Required: SIDE=<long|short> SIZE=<ngn_amount> COLLATERAL=<amount>");
        return;
    }

    const isLong = side === 'long';
    const sizeDeltaUsd = ethers.utils.parseUnits(size, 30); // Size in NGN

    // Collateral: mUSD for longs (6 decimals), mNGN for shorts (18 decimals)
    const collateralAmount = isLong ?
        ethers.utils.parseUnits(collateral, 6) :  // mUSD
        ethers.utils.parseUnits(collateral, 18);  // mNGN

    console.log(`\n${colors.bright}Opening ${side.toUpperCase()} Position:${colors.reset}`);
    console.log(`  Size: ${colors.green}${size} NGN${colors.reset}`);
    console.log(`  Collateral: ${colors.yellow}${collateral} ${isLong ? 'mUSD' : 'mNGN'}${colors.reset}`);

    // Calculate leverage
    if (isLong) {
        const leverage = parseFloat(size) / (parseFloat(collateral) * 1500);
        console.log(`  Leverage: ${colors.magenta}${leverage.toFixed(1)}x${colors.reset}`);
    } else {
    ...
    if (isLong) {
        // For opening longs, set max acceptable price (e.g., 1600 NGN per mUSD)
        acceptablePrice = ethers.utils.parseUnits("1600", 24); // 1600 with 24 decimals (30 - 6 for mUSD)
        console.log(`  ${colors.cyan}Setting acceptablePrice to 1600 NGN per mUSD for long open${colors.reset}`);
    } else {
...
async function increasePosition(signer, options) {
    ...
    const dataStore = await ethers.getContractAt("DataStore", ADDRESSES.DATA_STORE);
    const isLong = side === 'long';
    const collateralToken = isLong ? ADDRESSES.mUSD : ADDRESSES.mNGN;

    const positionKey = ethers.utils.keccak256(
        ethers.utils.defaultAbiCoder.encode(
            ["address", "address", "address", "bool"],
            [signer.address, ADDRESSES.MARKET, collateralToken, isLong]
        )
    );
    ...
    console.log(`  Additional Size: ${colors.green}${size} NGN${colors.reset}`);
    console.log(`  Additional Collateral: ${colors.yellow}${collateral} ${isLong ? 'mUSD' : 'mNGN'}${colors.reset}`);

    // Set acceptablePrice for position increases
    let acceptablePrice = 0;
    if (isLong) {
        acceptablePrice = ethers.utils.parseUnits("1600", 24); // 1600 NGN per mUSD
        console.log(`  ${colors.cyan}Setting acceptablePrice to 1600 NGN per mUSD for long increase${colors.reset}`);
    } else {
        acceptablePrice = 0;
...
async function decreasePosition(signer, options) {
    ...
    if (percent) {
        ...
        console.log(`\n${colors.bright}Decreasing ${side.toUpperCase()} Position by ${amount} NGN:${colors.reset}`);
    } else {
        // Default to 50%
    ...
    console.log(`  Current Size: ${colors.yellow}${ethers.utils.formatUnits(currentSize, 30)} NGN${colors.reset}`);
    console.log(`  Decrease Amount: ${colors.red}${ethers.utils.formatUnits(sizeDeltaUsd, 30)} NGN${colors.reset}`);
    ...
    const collateralDecimals = isLong ? 6 : 18;
    const collateralSymbol = isLong ? 'mUSD' : 'mNGN';
    console.log(`  Collateral to Withdraw: ${colors.green}${ethers.utils.formatUnits(collateralToWithdraw, collateralDecimals)} ${collateralSymbol}${colors.reset}`);

    // Set acceptablePrice based on position type
    let acceptablePrice = 0;
    if (isLong) {
        // For decreasing longs, use min acceptable price (e.g., 1400 NGN per mUSD)
        acceptablePrice = ethers.utils.parseUnits("1400", 24); // 1400 NGN per mUSD
        console.log(`  ${colors.cyan}Setting acceptablePrice to 1400 NGN per mUSD for long decrease${colors.reset}`);
    } else {
        // For decreasing shorts, set max acceptable price
        acceptablePrice = ethers.utils.parseUnits("1600", 24); // 1600 NGN per mUSD
        console.log(`  ${colors.cyan}Setting acceptablePrice to 1600 NGN per mUSD for short decrease${colors.reset}`);
    }
...
async function closePosition(signer, options) {
    ...
    console.log(`  Position Size: ${colors.yellow}${ethers.utils.formatUnits(currentSize, 30)} NGN${colors.reset}`);
    console.log(`  Collateral: ${colors.green}${ethers.utils.formatUnits(currentCollateral, collateralDecimals)} ${collateralSymbol}${colors.reset}`);
    ...
    if (isLong) {
        ...
        acceptablePrice = ethers.utils.parseUnits("1400", 24); // 1400 NGN per mUSD
        console.log(`  ${colors.cyan}Setting acceptablePrice to 1400 NGN per mUSD for long close${colors.reset}`);
    } else {
        // For closing shorts, set max acceptable price
        acceptablePrice = ethers.utils.parseUnits("1600", 24); // 1600 NGN per mUSD
        console.log(`  ${colors.cyan}Setting acceptablePrice to 1600 NGN per mUSD for short close${colors.reset}`);
    }
...
async function setPrice(signer, options) {
    ...
    if (!token || !price) {
        ...
        console.log("Required: TOKEN=<mUSD|mNGN> PRICE=<ngn_value>");
        return;
    }
    ...
    const tokenAddress = token.toUpperCase() === 'MUSD' ? ADDRESSES.mUSD : ADDRESSES.mNGN;

    // Convert price to proper precision
    let priceWithPrecision;
    if (token.toUpperCase() === 'MUSD') {
        // mUSD has 6 decimals, precision is 30
        // Price represents NGN per mUSD
        priceWithPrecision = ethers.utils.parseUnits(price, 24); // 30 - 6 = 24
    } else {
        // mNGN has 18 decimals, precision is 30
        // Price represents NGN per mNGN (should be 1)
    ...
    console.log(`  New Price: ${colors.green}${price} NGN${colors.reset}`);
...
async function main() {
    ...
    console.log(`\n${colors.bright}=== mUSD/mNGN Market Trading ===${colors.reset}`);
    ...
    const mngn = await ethers.getContractAt("IERC20", ADDRESSES.mNGN);
    ...
    console.log(`  mNGN: ${colors.green}${ethers.utils.formatUnits(mngnBalance, 18)}${colors.reset}`);
