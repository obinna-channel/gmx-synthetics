...
async function main() {
    console.log("=== CHECKING TSLA MARKET RATE CONFIGURATION ===\n");
    ...
    console.log("TSLA Market:", TSLA_MARKET);
    ...
    const baseBorrowingFactorKeyLong = ethers.utils.keccak256(
        ethers.utils.defaultAbiCoder.encode(
            ...
            [BASE_BORROWING_FACTOR, TSLA_MARKET, true]
        )
    );
    ...
    const baseBorrowingFactorKeyShort = ethers.utils.keccak256(
        ethers.utils.defaultAbiCoder.encode(
            ["bytes32", "address", "bool"],
            [BASE_BORROWING_FACTOR, TSLA_MARKET, false]
        )
    );
    ...
    const aboveOptimalKeyLong = ethers.utils.keccak256(
        ethers.utils.defaultAbiCoder.encode(
            ["bytes32", "address", "bool"],
            [ABOVE_OPTIMAL_USAGE_BORROWING_FACTOR, TSLA_MARKET, true]
        )
    );
    ...
    const aboveOptimalKeyShort = ethers.utils.keccak256(
        ethers.utils.defaultAbiCoder.encode(
            ["bytes32", "address", "bool"],
            [ABOVE_OPTIMAL_USAGE_BORROWING_FACTOR, TSLA_MARKET, false]
        )
    );
    ...
    const optimalUsageKeyLong = ethers.utils.keccak256(
        ethers.utils.defaultAbiCoder.encode(
            ["bytes32", "address", "bool"],
            [OPTIMAL_USAGE_FACTOR, TSLA_MARKET, true]
        )
    );
    ...
    const optimalUsageKeyShort = ethers.utils.keccak256(
        ethers.utils.defaultAbiCoder.encode(
            ["bytes32", "address", "bool"],
            [OPTIMAL_USAGE_FACTOR, TSLA_MARKET, false]
        )
    );
    ...
    const borrowingFactorKeyLong = ethers.utils.keccak256(
        ethers.utils.defaultAbiCoder.encode(
            ["bytes32", "address", "bool"],
            [BORROWING_FACTOR, TSLA_MARKET, true]
        )
    );
    ...
    const borrowingFactorKeyShort = ethers.utils.keccak256(
        ethers.utils.defaultAbiCoder.encode(
            ["bytes32", "address", "bool"],
            [BORROWING_FACTOR, TSLA_MARKET, false]
        )
    );
    ...
    const fundingExponentKey = ethers.utils.keccak256(
        ethers.utils.defaultAbiCoder.encode(
            ["bytes32", "address"],
            [FUNDING_EXPONENT_FACTOR, TSLA_MARKET]
        )
    );
    ...
    const borrowingExponentKeyLong = ethers.utils.keccak256(
        ethers.utils.defaultAbiCoder.encode(
            ["bytes32", "address", "bool"],
            [BORROWING_EXPONENT_FACTOR, TSLA_MARKET, true]
        )
    );
    ...
    // Create price payload (using placeholder price of $400 for TSLA)
    ...
    const marketInfo = await reader.getMarketInfo(DATA_STORE, marketPrices, TSLA_MARKET);
