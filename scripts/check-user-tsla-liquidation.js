...
async function main() {
    const TARGET_USER = "0x0E64500a73Ca057f6a002b55757EcC1Df31BfBfD";
    ...
    // Get current TSLA price (approximate - using $408 from the logs)
    const tslaPrice = ethers.utils.parseUnits("408.16", 12); // TSLA price with precision 12
    const usdPrice = ethers.utils.parseUnits("1", 24); // $1 with precision 24
    ...
    if (isLiquidatable) {
        ...
        console.log();
        console.log("💀 This position should be liquidated, not closed by the user!");
    } else {
        ...
        const remaining = parseFloat(ethers.utils.formatUnits(info.remainingCollateralUsd, 30));
        const minForLeverage = parseFloat(ethers.utils.formatUnits(info.minCollateralUsdForLeverage, 30));
        const buffer = remaining - minForLeverage;

        console.log();
        console.log("💡 Health Buffer:", buffer.toFixed(2), "USD");
        console.log("   (Amount above minimum required collateral)");
        console.log();
        console.log("⚠️  Maximum safe withdrawal:", buffer.toFixed(2), "USD");
        console.log("   (User should close with collateral withdrawal <=", buffer.toFixed(2), "USD)");
    }
